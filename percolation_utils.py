...
def bond_percolation(graph, p, n_trials=500):
    ...
    Returns:
        (crossing cluster probability psi(p))
    """
    percolate_count = 0
    n = len(graph.nodes())
    ...
    for _ in range(n_trials):
        ...
        # Check if any component crosses
        cross = any(_check_crossing_2d(comp, max_x, max_y) for comp in components)
...
def _check_crossing_2d(component, max_x, max_y):
    """
    Check if a component crosses the 2D lattice.
    ...
    Returns:
        True if component crosses, False otherwise
    """
    if not component:
        return False
    ...
    # crosses if touches both x-boundaries
    return ((min(x_coords) == 0 and max(x_coords) == max_x))
...
def plot_phase_transition(graph, title, n_points=15, n_trials=300):
    ...
    ax.set_ylabel('Probability of Existence of Crossing Cluster $\\psi(p)$', fontsize=13, fontweight='normal')
...
def plot_percolation_configs(graph, p_values, title, pos=None, p_c=None):
    ...
    for ax, p in zip(axes, p_values):
        ...
        # Find crossing cluster
        components = list(nx.connected_components(g))
        crossing_cluster = set()
        for component in components:
            if _check_crossing_2d(component, max_x, max_y):
                crossing_cluster = component
                break

        node_colors = ['#C41E3A' if node in crossing_cluster else '#CCCCCC'
